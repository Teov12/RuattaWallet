...
onMounted(async () => {
  ...
  await getAllTransactions().then(() => (isLoading.value = false));
});
</script>
...
<template>
  ...
  <div v-else>
    ...
    <div class="container">
      <div class="card">
        <h3 class="card-header">Compras</h3>
        <div class=" card-body container d-flex align-items-center justify-content-center" v-if="transactions.purchases.length == 0">
          <h5 class="card-title">No se han efectuado compras hasta el momento</h5>
        </div>
          ...
          <table class="table container text-center">
            ...
            <tbody>
              <tr v-for="(item, i) in transactions.purchases" :key="i">
                ...
                <td>{{ item.money }}</td>
                <button type="button" @click="() => (selectedTrans = item)" class="btn btn-primary m-2" data-bs-toggle="modal" data-bs-target="#exampleModal">Ver</button>
                <RouterLink @click="() => getTransaction(item._id)" class="btn btn-success m-2" :to="`/history/update/${item._id}`"> Modificar</RouterLink>
      ...
      <div class="container mt-5">
        <div class="card">
          <h3 class="card-header">Ventas</h3>
          <div class="card-body container d-flex justify-content-center" v-if="transactions.sales.length == 0">
            <h5 class="card-title">No se han efectuado ventas hasta el momento</h5>
          </div>
          <div class="card-text" v-else>
